...
if __name__ == "__main__":
    ...
    print("\nBest Selection:")
    best_individuals = Selection.best_selection(population, num_selected=3, selection_type=selection_type)
    for individual in best_individuals:
        print(f"Fitness: {individual.fitness}, Decoded: {individual.decoded_variables}")

    # Testing roulette selection
    print("\nRoulette Selection:")
    selected_by_roulette = Selection.roulette_selection(population, num_selected=3, selection_type=selection_type)
    for individual in selected_by_roulette:
        print(f"Fitness: {individual.fitness}, Decoded: {individual.decoded_variables}")

    # Testing tournament selection
    print("\nTournament Selection:")
    selected_by_tournament = Selection.tournament_selection(population, num_selected=3, selection_type=selection_type)
